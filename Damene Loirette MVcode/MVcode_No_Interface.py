...
def bin_word_list(messageInit):
    #Prend en entrée le message initial et retourne la une liste d'octets qui correspond au message + la correction
    final_List = []
...
def Start():
    ...
    while redondance == None:
        ...
        print("--------------------------------------------------------------------\n")
        tmp = input("Entrez la valeur de la redondance : ")
        if int(tmp) <= 0 or int(tmp) > 8 : print("Valeur pas bonne")
        else: redondance = int(tmp)
    
    while messageInit == None:
        print("Entrez votre message (limite = ",MAXLIST[redondance-1],"caracteres ) : ")
        tmp = input()
        if len(tmp) <= 0  or len(tmp) > MAXLIST[redondance-1]: 
            print("Message incorect , Votre message contient ",len(tmp)," Caracteres")
        else:
            messageInit = tmp
